...
class DNN2(pl.LightningModule):
    ...
    def __init__(self, metric=MeanSquaredError, lr_rate=0.001, seed=None):  # low lr to avoid overfitting
        ...
        self.u_age = nn.Embedding(n_ages,n_factors)
        self.u_occup = nn.Embedding(n_occupatioons,n_factors)


        self.drop = nn.Dropout(embedding_dropout)
        self.hidden = nn.Sequential(
            nn.Linear(n_factors * 4 + 19, 128),
            nn.ReLU(),
            nn.Dropout(dropouts),
            nn.Linear(128, 64),
            nn.ReLU(),
    ...
    def forward(self, users, movies, ages, occups, movie_genres, minmax=None):
        """
        """
        features = torch.cat([self.u(users), self.m(movies), self.u_age(ages),
                              self.u_occup(occups),movie_genres], dim=1)
    ...
    def training_step(self, batch, batch_id):
        ...
        outputs = self(x[:, 0], x[:, 1], x[:,2], x[:,3], x[:,4:], minmax=minmax)
